...
/*!
		\brief Функция для записи байта в бинарный файл
		\details Эта функция упаковывает восемь битов в байт, 
		затем записывает его в бинарный файл. Последний байт
		может оказаться незаполненным, ввиду чего использование
		этой функции требует последующего вызова функции FlushBits()
		для вставки дополнительных нулей
		\param[in] bit бит (0 или 1)
		\param[in] outfile файл, открытый в бинарном режиме
*/
void WriteBit(int bit, std::ostream& outfile);
/*!
		\brief Функция для вставки дополнительных нулей
		\details Функция writeBinaryString() может не записать биты,
		в случае если исходная последовательность не делится без 
		остатка на ноль. Для того, чтобы избежать эту ошибку последний
		байт заполняется недостающими нулями
		\param[in] outfile файл, открытый в бинарном режиме
*/
